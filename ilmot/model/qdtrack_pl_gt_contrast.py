...
class QDTrackPLGTContrast(ILMOTBaseModel):  # type: ignore
        ...
        for key, value in loaded_state_dict.items():
            if not load_vist:
                ...
                if key.startswith("student_detector."):
                    ...
                    key = key[16:]
                    loaded_similarity_state_dict[key] = value
        if not load_vist and ("embedding_means" in loaded_state_dict.keys()):
    ...
    def prepare_embeddings(self, embeddings, sample_result):
        """Prepare embeddings for the contrast loss."""
        # sample pos and negs
        new_class_embeddings = []
        for embedding, result in zip(embeddings, sample_result):
            gt_label = result.pos_gt_labels
            new_class_mask = gt_label >= self.num_old_cat
            embedding = embedding[new_class_mask, :]
            new_class_embeddings.append(embedding)
        new_class_embeddings = torch.cat(new_class_embeddings, dim=0)

        pos_num = int(self.sample_num * self.sample_pos_frac)
        pos_inds = torch.nonzero(
            self.memory_label >= self.num_old_cat,  # type:ignore
            as_tuple=False,
        )
        ...
        pos_num = min(pos_num, pos_inds.numel())
        neg_num = min(self.sample_num - pos_num, neg_inds.numel())

        pos_perm = torch.randperm(pos_inds.numel())[:pos_num]
        pos_inds = pos_inds[pos_perm].squeeze(1)
        neg_perm = torch.randperm(neg_inds.numel())[:neg_num]
        neg_inds = neg_inds[neg_perm].squeeze(1)

        pos_embedding = self.memory_embedding[pos_inds, :]  # type:ignore
        neg_embedding = self.memory_embedding[neg_inds, :]  # type:ignore

        contrast_embedding = torch.cat([pos_embedding, neg_embedding], dim=0)
        dists = cosine_similarity(new_class_embeddings, contrast_embedding)

        contrast_target = torch.cat(
            [torch.ones(pos_num), torch.zeros(neg_num)]
        )
        contrast_target = contrast_target.to(
            dtype=torch.int, device=dists.device
        )
        contrast_target = contrast_target.unsqueeze(0).repeat(
            dists.shape[0], 1
        )

        return dists, contrast_target
