...
class QDTrackPLGTContrastBoth(ILMOTBaseModel):  # type: ignore
    ...
    def __init__(
        ...
        **kwargs: ArgsType,
        ...
        self.track_embedding_length = 256
    ...
    def forward_train(
        ...
        if self.enable_track_contrast:
            ...
            track_embeddings = self.similarity_head._head_forward(
                key_x, sampled_proposals
            )

            self.similarity_head.eval()
            with torch.no_grad():
                track_embeddings_contrast = self.similarity_head._head_forward(
                    key_x, sampled_proposals
                )
        ...
        for cat_id in range(-1, len(self.category_mapping)):
            ...
            if self.track_memory_num[cat_id] >= self.memory_per_class:
                ...
                current_class_std = torch.sqrt(
                    torch.mean(
                        (
                            self.track_memory_embedding[cat_id, :  self.track_memory_num[cat_id], :]
                            - self.track_embedding_means[cat_id, :]
                        )
                        ** 2,
                        dim=0,
