...
class BaseEvaluatorCallback(Callback):
    ...
    def gather(self, pl_module: pl.LightningModule) -> None:
        ...
        preds = all_gather_predictions(self._predictions, pl_module, self.collect)
    ...
    def process(self, inputs: List[List[InputSample]], outputs: ModelOutput) -> None:
    ...
    def evaluate(self, epoch: int) -> Dict[str, MetricLogs]:
        ...
        for key, predictions in self._predictions.items():
            if self.output_dir:
                ...
                file_path = os.path.join(self.output_dir, f"{key}_predictions.json")
